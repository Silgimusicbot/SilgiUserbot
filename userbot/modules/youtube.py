    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(COOKIES_URL) as resp:
                if resp.status != 200:
                    return None, f"❌ Cookies xətası: {resp.status}"
...
async def ytaudio(event):
    ...
    if not query:
        await event.edit("ℹ️ Mahnı adı və ya link daxil edin.")
        return
    ...
    try:
        ...
        if cookies_path and os.path.exists(cookies_path): os.remove(cookies_path)
...
async def ytvideo(event):
    query = event.pattern_match.group(1).strip()
    if not query:
        await event.edit("ℹ️ Video adı və ya link daxil edin.")
        return
...
CmdHelp("youtube").add_command("ytmp3", "ad/link", "MP3 yükləyir.").add_command("ytvideo", "ad/link", "Video yükləyir.").add_sahib("[SILGI](https://t.me/silgiteam)").add()
